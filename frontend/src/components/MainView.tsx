            ...
            {mobileAppEnabled && (
              <div className="settings-section">
                ...
                {(currentUser?.isAdmin || !authEnabled) ? (
                  <>
                    {mobileApkAvailable ? (
                      <button type="button" className="btn-sm btn-sm-primary" style={{ marginTop: '0.5rem' }} onClick={handleDownloadApk}>{isNativeApp ? 'Update app' : 'Download Android app (APK)'}</button>
                    ) : (
