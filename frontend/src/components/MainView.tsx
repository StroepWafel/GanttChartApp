        ...
        {settingsTab === 'app' && (mobileAppEnabled || currentUser?.isAdmin || !authEnabled) && (
          <div className="settings-tab-content" role="tabpanel">
            {mobileAppEnabled && (
              <div className="settings-section">
                <h4>Mobile app</h4>
                <p className="settings-desc">Download the native Android app for Gantt Chart. Built with Capacitor.</p>
                {(currentUser?.isAdmin || !authEnabled) ? (
                  <>
                    {mobileApkAvailable ? (
                      <a href={api.APK_DOWNLOAD_URL} className="btn-sm btn-sm-primary" style={{ display: 'inline-block', marginTop: '0.5rem' }} download="gantt-chart.apk">{isNativeApp ? 'Update app' : 'Download Android app (APK)'}</a>
                    ) : (
                      <p className="settings-desc" style={{ marginTop: '0.5rem', fontSize: '0.85rem', color: 'var(--muted)' }}>APK available after build. Use &quot;Build now&quot; below or run the GitHub workflow.</p>
                    )}
                    <p className="settings-desc" style={{ marginTop: '0.75rem', fontSize: '0.85rem' }}>APK is generated by the build or via the GitHub workflow.</p>
                  </>
                ) : (
                  <>
                    {mobileApkAvailable && (
                    ...
                    <p className="settings-desc" style={{ marginTop: '0.75rem', fontSize: '0.85rem' }}>APK is generated by the server.</p>
                  </>
                )}
              </div>
            )}
    ...
    <div className="main-view">
      ...
      {isMobile && !isNativeApp && mobileAppEnabled && mobileApkAvailable && !mobileAppPromptDismissed && (
        <div className="mobile-app-prompt-banner" role="status">
          ...
          <div className="mobile-app-prompt-actions">
            <a href={api.APK_DOWNLOAD_URL} className="btn-sm btn-sm-primary" download="gantt-chart.apk">Download app</a>
      ...
      {isNativeApp && mobileAppUpdateAvailable && (
        <div className="mobile-app-prompt-banner mobile-app-update-banner" role="status">
          ...
          <a href={api.APK_DOWNLOAD_URL} className="btn-sm btn-sm-primary" download="gantt-chart.apk">Update app</a>
        </div>
      )}
